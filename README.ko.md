...
![build234의 f0r h3r 일곱 섹션과 분리된 테이크·영상 녹음 조작](docs/images/circlr-song-build234.png)

*0.80.0 build234 개발 미리보기: 격리된 실제 앱 화면입니다. 최신 정식판은 0.70.0입니다. [촬영 정보](docs/images/README.md).*

> **0.70.0 · build180.** 검증한 조건에서 재생 캔버스와 MP4 캡처를 개선하고, 감상 모드를 녹화 버튼 옆에 놓았습니다. [증거와 제한](docs/releases/0.70.0-qa.md).

**개발 중:** [0.80.0](docs/releases/0.80.0.md)은 곡 궤도에 섹션 순번·시간을 표시하고, 단일 곡을 섹션 화면으로 열며 이름표가 타임라인과 콘솔 조작을 가리지 않게 합니다. build234는 MIDI 반복 녹음이 늦은 이벤트를 요청한 반복 수의 끝에서 자르고, 상단의 MIDI/오디오 테이크 녹음과 MP4 영상 녹화를 구별합니다. 선택한 MIDI 편집·바운스만 다루는 앱 소유 오프라인 MCP helper도 패키징하지만, 이는 앱 내 Codex 계정 대화가 아닙니다. 앞선 격리 앱에서 외부 Codex 모델의 MIDI 편집·Undo·WAV 내보내기는 MCP로 확인했습니다. 실제 장치 take·청취·장시간 녹화 QA는 열려 있으며 최신 정식판은 0.70.0입니다. [현재 QA](qa/0.80-build234.md).
...
<img src="docs/images/circlr-song-portrait-build234.png" width="280" alt="build234 세로 창에서 일곱 섹션과 분리된 테이크·영상 녹음 조작">

*같은 build234 데모를 정지한 720×900 세로 창입니다. 콘솔을 펼친 상태에서 팔로우 대상, 두 줄 상단 녹음 조작과 일곱 섹션을 보여줍니다. [촬영 정보](docs/images/README.md).*
...
구형 오디오 인터페이스에서 재생이 시작되지 않으면 circlr에서 Mac 내장 출력을 선택하고 [오디오 장치 안내](docs/install-macos.md)를 확인하세요.
...
빌드는 앱·오디오 helper 5개·내부 trusted MCP helper·에이전트 키트를 함께 패키징합니다. 로컬 패키지는 ad-hoc 서명이며 공증되지 않았습니다. 개발 버전을 시험할 때 중요한 프로젝트는 사본으로 보관하세요.
...
1. 빈 캔버스에서 우클릭해 서클을 만듭니다. 섹션으로 곡을 설계하고 궤도 위와 주변에 MIDI·오디오·악기·이펙터를 배치합니다.
