...
![컬러를 적용한 f0r h3r의 곡 궤도와 연결된 섹션들](docs/images/circlr-song-build195.png)

*0.80.0 build195 개발 미리보기: 실제 검증 앱의 MP4 첫 프레임입니다. 최신 정식판은 0.70.0입니다. [촬영 정보](docs/images/README.md).*

> **0.70.0 · build180.** 검증한 조건에서 재생 캔버스와 MP4 캡처를 개선하고, 감상 모드를 녹화 버튼 옆에 놓았습니다. [증거와 제한](docs/releases/0.70.0-qa.md).

**개발 중:** [0.80.0](docs/releases/0.80.0.md)은 궤도 가독성을 높이고 재생·녹화·편집·출력의 전체 작업 흐름을 검증합니다. build204에서는 세로형 창의 케이블 조작을 두 줄로 배치하고, 구버전 복구본을 보존하며 다른 앱의 로컬 MCP socket을 교체하지 않도록 게시합니다. MIDI·오디오 편집 화면 유지, MCP 작업별 취소, **⌘,**의 앱 전용 출력 장치 선택도 개발 중입니다. 필수 QA가 열려 있으며 최신 정식판은 0.70.0입니다.
...
*정식판 build180의 별도 f0r h3r 세로 화면입니다. MP4는 캔버스만 담아 상단 컨트롤바는 보이지 않습니다. [촬영 정보](docs/images/README.md).*
