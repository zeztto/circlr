...
![build233의 f0r h3r와 읽기 쉬운 일곱 섹션을 보여 주는 곡 궤도](docs/images/circlr-song-build233.png)

*0.80.0 build233 개발 미리보기: 격리된 실제 앱 화면입니다. 최신 정식판은 0.70.0입니다. [촬영 정보](docs/images/README.md).*

> **0.70.0 · build180.** 검증한 조건에서 재생 캔버스와 MP4 캡처를 개선하고, 감상 모드를 녹화 버튼 옆에 놓았습니다. [증거와 제한](docs/releases/0.70.0-qa.md).

**개발 중:** [0.80.0](docs/releases/0.80.0.md)은 곡 궤도에 섹션 순번·시간을 표시하고, 단일 곡을 섹션 화면으로 열며 이름표가 타임라인과 콘솔 조작을 가리지 않게 합니다. build233은 다른 MIDI 채널과 재트리거에서 겹치는 음을 녹음 take에 보존하고, 좁은 창에도 팔로우 대상을 표시하며, 에이전트용 간결한 읽기 도구 `circlr_context`를 추가합니다. 별도 build230 앱에서 외부 Codex 모델의 제한된 MIDI 편집·Undo·WAV 내보내기를 MCP로 검증했지만 앱 내 계정 대화는 아직 없습니다. 실제 장치 take·청취·장시간 녹화 QA는 열려 있으며 최신 정식판은 0.70.0입니다. [현재 QA](qa/0.80-build233.md).
...
<img src="docs/images/circlr-song-portrait-build233.png" width="280" alt="build233 세로 창에서 일곱 섹션과 가려지지 않은 궤도 눈금">

*같은 build233 데모를 정지한 720×900 세로 창입니다. 콘솔을 펼친 상태에서 팔로우 대상, 두 줄 상단 조작, 가까운 섹션 이름표와 일곱 섹션을 보여줍니다. [촬영 정보](docs/images/README.md).*
