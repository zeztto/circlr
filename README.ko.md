...
> **개발 프리뷰입니다.** 0.30.0은 개발 중이며 릴리스가 완료되지 않았습니다. 실제 오디오 출력·한글 IME 입력·일부 통합 검증이 남아 있습니다. 제작에 사용하기 전 [현재 QA 기록](docs/releases/0.30.0-qa.md)을 확인하세요.
...
써클러는 향후 오픈소스 공개를 준비하고 있습니다. **아직 오픈소스 라이선스를 선정하거나 추가하지 않았습니다.** 현재 공개·재사용 권한이 부여된 것으로 간주하지 마세요. 외부 샘플과 비공개 음악 프로젝트는 이 저장소에 포함하지 않습니다.
