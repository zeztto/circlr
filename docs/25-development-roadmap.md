...
같은 build122의 [연속 제작 검증](141-production-flow-validation.md)에서 바운스 전후 PCM과 Undo/재열기를 확인하고 원본 복원 후의 화면 복귀 문제를 다음 개선으로 확정했다.


최신 build122는 한 곡 동선 점검에서 확인한 공유 MIDI의 ⌘4 대상 변경과 탐색 누락을 수정했다. [검증 기록](140-step-target-navigation.md). 전체 제작 흐름은 계속 검증한다.
