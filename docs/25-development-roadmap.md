...
갱신: 2026-09-10. 계획 시작 기준: 0.11 native 앱, 0.12 음악 에이전트 키트 소스. 사용 앱은 0.19.0, 개발 검증 후보는 0.20.0 build 84이다. 목표는 송폼 중심의 전문 음악 제작을 먼저 완성하고, 이를 아티스트의 작품·세계관 관리로 확장하는 것이다.
...
다음 우선과제는 바운스의 대상·거절 이유·현재 서클의 경로 포함 여부를 편집 화면에서 확인하고 연결 수정으로 이동하는 동선이다. 소스 감사에서 `TrackBounceButton.swift:17–21`의 비활성 ‘바운스·연결 확인’은 실제 원인을 help에만 제공하며, `BounceEditing.swift:23–26`은 트랙 출력을 기준으로 판단함을 확인했다. 같은 트랙에서 오디오 A만 연결하고 B를 편집하면 A 경로 바운스가 가능하되 B 제외가 UI에 드러나지 않을 수 있다. 이는 소스 감사 결과이며 native에서 확정한 결함은 아니다.

먼저 이 동선을 실제 앱에서 재현한다. 재현 결과를 기준으로 typed bounce issue/destination, inline 거절 원인·현재 서클 경로 밖 표시, 활성 출력 연결 보기 동작으로 수정 대상에 바로 이동하는 범위를 정한다. 오디오 가져오기→편집→오토메이션→바운스 도구 왕복에서 현재 편곡·편집 대상과 복귀 비용을 함께 확인한다. 실제 A/B 청취와 출력의 간헐적 첫 연결 실패는 별도 재생 조건으로 유지한다.
