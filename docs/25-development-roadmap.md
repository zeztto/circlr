...
0.13에서 A/B와 D의 구현·수치/바운스 검증을 마쳤고 C의 녹화 경로와 코덱 검사를 추가했다. 다음 최우선 항목은 Scarlett 출력 연결의 재현·장치 변경 대응, 실제 녹화의 재생 동기·최소화·종료 검증, f0r h3r v2 청취 피드백이다. [현재 검증 상태](../qa/0.13-review.md)를 기준으로 E 이후의 범위를 진행한다.
