...
갱신: 2026-09-09. 계획 시작 기준: 0.11 native 앱, 0.12 음악 에이전트 키트 소스. 사용 앱은 0.19.0, 개발 검증 후보는 0.20.0 build 70이다. 목표는 송폼 중심의 전문 음악 제작을 먼저 완성하고, 이를 아티스트의 작품·세계관 관리로 확장하는 것이다.
...
다음은 선택 노트/오토메이션 점/오디오 분할 커서의 서클별 복귀와 작은 창의 오디오 정밀 입력·작업 버튼 가시성을 정리한다. 보기 저장은 유효한 ID·asset/대상 변경을 검사하며 음악 Undo와 분리한다. build70에서 스텝 행 AX 프레임과 스크롤 중 객체 교체를 수정했다. 실제 장치 재생·입력·VoiceOver 발화와 사용 앱 출고는 별도 완료 조건이다.
...
현재 전달 조건은 기능별로 구분한다. UI·편집 개선은 build 70 QA 앱에서 직접 검토할 수 있다. 사용 중인 0.19 앱을 교체하려면 우선 같은 Mac에서 출력 연결→실제 재생/정지→재시작과 기존 곡/MP4 회귀를 끝내야 한다. 녹음은 별도 허용이 필요한 실제 입력·취소·테이크 저장 회귀가 남았다. VoiceOver와 밀집 연결 조작은 전역의 모든 조합이라는 무한 조건 대신 MIDI/audio/sidechain/flow, 접힌 그룹, 긴 이름, 작은 창의 대표 경로를 명시한 검사표로 좁혀 수행한다. 과거 QA 수치를 새 빌드의 전체 기능 승인으로 합산하지 않는다.
