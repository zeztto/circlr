...
갱신: 2026-09-09. 계획 시작 기준: 0.11 native 앱, 0.12 음악 에이전트 키트 소스. 사용 앱은 0.19.0, 개발 검증 후보는 0.20.0 build 55이다. 목표는 송폼 중심의 전문 음악 제작을 먼저 완성하고, 이를 아티스트의 작품·세계관 관리로 확장하는 것이다.

**build 55에서 음악 위치의 표시 기준을 통일했다.** 가져오기·오디오·MIDI·오토메이션·부모 안 시작이 첫 위치 1박을 사용한다. 길이/초·모델/MCP·원시 정밀도는 보존하며 오류와 접근성 설명도 일치한다. Swift 406개·Python 26개, 실제 입력·편집 방식 왕복·가져오기·Undo·재열기와 소스/앱 검사는 [계약](69-beat-position-display.md) · [QA](../qa/beat-position-review.md)에 있다. 다음 UI 우선순위는 궤도/자유 배치 전환을 음악 Undo와 분리하는 계약이다. 현재는 보기 전환이 Undo를 먼저 소비하며 이전 음악 snapshot의 보기까지 복원한다. 보기 선호의 저장·dirty 상태·음악 Undo/Redo·MCP revision을 함께 검토하고, 실제 위치/연결 배치 변경의 Undo와 구분해야 한다.
...
현재 전달 조건은 기능별로 구분한다. UI·편집 개선은 build 55 QA 앱에서 직접 검토할 수 있다. 사용 중인 0.19 앱을 교체하려면 우선 같은 Mac에서 출력 연결→실제 재생/정지→재시작과 기존 곡/MP4 회귀를 끝내야 한다. 녹음은 별도 허용이 필요한 실제 입력·취소·테이크 저장 회귀가 남았다. VoiceOver와 밀집 연결 조작은 전역의 모든 조합이라는 무한 조건 대신 MIDI/audio/sidechain/flow, 접힌 그룹, 긴 이름, 작은 창의 대표 경로를 명시한 검사표로 좁혀 수행한다. 과거 QA 수치를 새 빌드의 전체 기능 승인으로 합산하지 않는다.
