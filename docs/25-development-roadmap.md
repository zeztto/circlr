...
다음 실행 순서는 다음과 같다. (1) 최초 출력 연결이 CoreAudio HAL에서 대기하는 경로를 장치 변경 없이 계측하고, 정지·재시도·종료 시 진행 상태와 단일 연결 작업의 수명을 명확히 한다. 성공 조건은 실제 프로세스의 준비/실패 상태와 지연 원인을 구분하고 UI를 계속 조작할 수 있는 것이다. (2) 같은 build의 궤도 배치·다수 섹션 전환·펼친 그룹·긴 이름·시간 손잡이·VoiceOver 조합을 점검한다. 이름표/포트 hit와 그려진 위치가 일치하고 키보드로 편집/복귀가 가능해야 한다. (3) 공통 파일 drop과 로컬 라이브러리를 연결해 import→섹션 배치→편집→바운스→저장 복원의 작업 깊이를 줄인다. 원본 참조·중복 자산·Undo 계약을 먼저 정한다. 마이크 입력의 별도 실행 조건과 E 출고 gate는 유지한다.
...
| B | 생성/명령 검색/⌘J/트랙 전환/8방향 연결·재생 follow 확대와 라벨 개선 | 궤도/펼친 그룹/긴 이름 밀집 조합·VoiceOver |
