...
작성: 2026-09-08. 계획 시작 기준: 0.11 native 앱, 0.12 음악 에이전트 키트 소스. 현재 산출물은 0.13.0이다. 목표는 송폼 중심의 전문 음악 제작을 먼저 완성하고, 이를 아티스트의 작품·세계관 관리로 확장하는 것이다.

0.13에서 A/B와 D의 구현·수치/바운스 검증을 마쳤고 C의 녹화 경로와 코덱 검사를 추가했다. 배포용 f0r h3r는 CC0 샘플로 교체한 v3를 사용한다. 다음 최우선 항목은 Scarlett 출력 연결의 재현·장치 변경 대응, 실제 녹화의 재생 동기·최소화·종료 검증, v3 청취 피드백이다. [현재 검증 상태](../qa/0.13-review.md)를 기준으로 E 이후의 범위를 진행한다. [Splice 연동 계획](27-splice-licensing-and-integration.md)은 공통 파일 import → 로컬 라이브러리 → companion AU 순서로 진행한다.
...
단일 agent 슬롯에서 UX → 구현 → 코드/보안 검토 → QA를 순차 수행한다. 서로 다른 파일 책임을 명시하고 사용자 변경을 덮어쓰지 않는다. 넓은 요구를 문서에만 남기고 종료하지 않고, 현재 환경에서 가능한 B/C/D를 이어 구현한다. 실제 진행과 남은 항목은 `qa/0.13-progress.md`에 갱신한다. 날짜 약속 대신 검증 완료 조건으로 다음 단계를 시작한다.
