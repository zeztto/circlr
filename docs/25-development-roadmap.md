...
갱신: 2026-09-10. 계획 시작 기준: 0.11 native 앱, 0.12 음악 에이전트 키트 소스. 사용 앱은 0.19.0, 개발 검증 후보는 0.20.0 build 83이다. 목표는 송폼 중심의 전문 음악 제작을 먼저 완성하고, 이를 아티스트의 작품·세계관 관리로 확장하는 것이다.

## 현재 작업 — 편곡 대안의 복제·이름 변경
...
다음 사용성 과제는 이름과 섹션 수가 같은 안의 차이를 보여주는 것이다. 실제 재생 경로의 순서·반복·경로 밖 섹션을 목록에서 구분하고, 섹션 연결/순서 변경 후 원안과 대안을 오가는 동선을 검증한다. 실제 A/B 청취와 출력의 간헐적 첫 연결 실패는 별도 재생 조건으로 유지한다.
