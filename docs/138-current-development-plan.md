...
기준: 2026-09-10, integration-worktree의 build120 소스(`b06c4a9`)에서 시작해 build127의 소스·검증 기록을 기준으로 build136 검증 결과까지 반영했다. 이 문서는 다음 실행과 완료 판단을 위한 계획이며 [누적 로드맵](25-development-roadmap.md)의 과거 검증 결과를 새로 수행한 검사로 바꾸지 않는다. 전체 DAW·음악 품질·접근성은 아직 완료되지 않았다.
...
| 편집 접근 | build116 공유 리듬 오디오, build117 오디오 수치 접근, build119 신스 포커스 노출, build120 콘솔 설정 복원. build136 네 MIDI 모드의 직접 파일 작업·수치 확정·취소 복귀·작은 창 배치 및 Core 13개 검증 | 한 곡 전체의 연속 사용성, 모든 폼·최소 높이·VoiceOver·IME 조합 |
