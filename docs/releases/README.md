...
상태: 운영 규약 확정. 시작 기준은 `0.20.0 build152`, 현재 정식 출고는 **0.60.0 build170** (`v0.60.0`)이다. 출고 여부는 버전 문서의 gate로 판단한다.

## 문서와 버전의 기준

- 현재 버전 계획: [0.70.0 재생 비주얼라이저 성능](0.70.0.md) · [진행 중 QA](0.70.0-qa.md). 최신 정식판 기록: [0.60.0 계획](0.60.0.md) · [한영 노트](0.60.0-notes.md) · [QA](0.60.0-qa.md).
...
초기 버전은 `codex/daw-integration`에서 `release/0.30.0`을 만들었다. 이후 버전은 직전 릴리스 tag를 기준으로 `release/<version>`에서 개발한다. 현재 0.70.0은 `release/0.70.0`에 있으며, 기존 커밋·브랜치를 재작성하지 않는다. 버전 중간에는 복구용 로컬 commit 또는 필요시 개발 브랜치 checkpoint push가 가능하지만 릴리스로 세지 않는다.
