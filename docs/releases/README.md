...
상태: 운영 규약 확정. 시작 기준은 `0.20.0 build152`, 현재 정식 출고는 **0.70.0 build180** (`v0.70.0`)이다. 이번 버전은 사용자의 출고 지시에 따라 미검증 native QA를 공개하고 게시했다. 출고 여부와 검증 범위는 버전 문서의 gate로 판단한다.

## 문서와 버전의 기준

- 현재 개발 계획: [0.80.0 궤도 UI·핵심 제작 안정화](0.80.0.md) → [0.90.0 장시간·통합 신뢰성](0.90.0.md). Codex 계정 대화는 공식 production 지원 경로 확인 전까지 별도 G0 backlog다. 최신 정식판 기록: [0.70.0 재생 비주얼라이저 성능](0.70.0.md) · [한영 노트](0.70.0-notes.md) · [QA와 미검증 범위](0.70.0-qa.md).
...
필수 QA 실패/미실행이면 RC 또는 IN_PROGRESS에 머문다. 환경 문제는 해당 gate와 필요한 입력을 기록하고 독립 작업을 진행한다. 완료를 위해 필수 항목을 임의로 선택 사항으로 바꾸지 않는다. 낮은 우선순위의 비차단 제한만 영향·재현·후속 버전을 명시해 수용할 수 있다. **0.70.0 한정 예외:** 사용자가 열린 QA를 알면서 정식 출고를 지시했다. OPEN 항목은 노트·QA·최종 안내에 남기며 PASS로 바꾸지 않는다. 이 예외는 다음 버전으로 자동 승계하지 않는다. ‘한 단계씩’의 종료 단위는 작은 수정이나 build가 아니라 위 절차를 마친 버전이다. 사용자 정지·질문·필수 외부 입력은 중간 중단 사유다.
