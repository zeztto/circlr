...
상태: 운영 규약 확정. 시작 기준은 `0.20.0 build152`, 현재 후보는 **0.50.0 build166**이며 `v0.50.0` 게시 대기다. 직전 공개 출고는 0.40.0 build162다. 출고 여부는 버전 문서의 gate로 판단한다.

## 문서와 버전의 기준

- 현재 버전 계획: [0.50.0 펼쳐지는 궤도와 화면 비율 대응](0.50.0.md). [한영 노트](0.50.0-notes.md) · [QA](0.50.0-qa.md).
