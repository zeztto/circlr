...
상태: 운영 규약 확정. 시작 기준은 `0.20.0 build152`, 최신 정식 출고는 **0.80.0 build242** (`v0.80.0`)이다. 2026-09-25 사용자 이월 결정으로 남은 영상 품질 FAIL·실기기 OPEN을 공개한 채 정식 게시한다. 출고 여부는 원격 Release·자산 검증으로, 검증 범위는 버전 문서의 원 판정으로 판단한다.

## 문서와 버전의 기준

- 현재 개발 계획: [0.90.0 장시간·통합 신뢰성](0.90.0.md). Codex 계정 대화는 공식 production 지원 경로 확인 전까지 별도 G0 backlog다. 0.80 출고 기록: [버전](0.80.0.md) · [한영 노트](0.80.0-notes.md) · [QA와 이월 범위](../../qa/0.80-release-gates.md).
...
필수 QA 실패/미실행이면 RC 또는 IN_PROGRESS에 머문다. 환경 문제는 해당 gate와 필요한 입력을 기록하고 독립 작업을 진행한다. 완료를 위해 필수 항목을 임의로 선택 사항으로 바꾸지 않는다. 낮은 우선순위의 비차단 제한만 영향·재현·후속 버전을 명시해 수용할 수 있다. **0.70.0 한정 예외:** 사용자가 열린 QA를 알면서 정식 출고를 지시했다. **0.80.0 한정 예외:** 2026-09-25 사용자는 핵심 DAW·AI 동선을 막지 않는 남은 영상 품질 및 실기기 검증을 다음 버전으로 이월하고 정식 버전을 닫도록 지시했다. 각 FAIL·OPEN은 노트·QA·최종 안내에 남기며 PASS로 바꾸지 않는다. 두 예외는 다음 버전으로 자동 승계하지 않는다. ‘한 단계씩’의 종료 단위는 작은 수정이나 build가 아니라 위 절차를 마친 버전이다. 사용자 정지·질문·필수 외부 입력은 중간 중단 사유다.
