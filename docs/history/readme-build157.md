# 이전 README — build157 시점 기록

이 문서는 README 개편 전 개발 이력을 보존한 기록입니다. 현재 지원 범위·빌드 안내는 [README](../../README.md), 현재 QA는 [0.30.0 기록](../releases/0.30.0-qa.md)을 따릅니다. 일부 과거 로컬 QA 산출물 링크는 배포되지 않습니다.

# 써클러(circlr)

**개발 운영:** [버전별 계획·완료 절차](../../docs/releases/README.md) · [0.30.0 상세 계획](../../docs/releases/0.30.0.md) · [이후 로드맵](../../docs/releases/roadmap.md). 현재 `release/0.30.0`에서 0.30.0 후보를 구현·검증 중이다. 필수 QA가 끝나기 전에는 릴리스하지 않는다. 기능 묶음별 구현·QA·독립 검토·문서·commit/tag/push를 마친 뒤 릴리스한다.

macOS용 앨범·송폼 궤도 편집기. **앨범 → 곡·악장 → 섹션 → MIDI·오디오·악기·이펙터**를 하나의 자유로운 캔버스에서 다룬다.

**0.30.0 개발 중:** MIDI 파일 읽기를 백그라운드로 옮기고 취소·접근 권한 수명을 보완했다. 페달 전환 시 키보드 포커스와 스텝 Tab 이동을 수정했다. build157에서 높은 콘솔의 페달 수치 가림과 Tab 자동 노출을 수정·재검증했다. 실제 출력·한글 IME 등 남은 조건은 [후보 검증 기록](../../docs/releases/0.30.0-qa.md)에 구분한다. 설치 앱은 교체하지 않았다.

**이전 소스 build152:** 페달 그래프 준비 중 포커스 요청을 유지하도록 수정했다. 별도 QA 앱에서 모드 전환·재열기 후 클릭 없는 Home/End 선택과 데이터 보존을 확인했다. 전환 직후 입력 타이밍은 남은 확인 항목이며 설치 앱은 교체하지 않았다. [변경·확인 절차](../../docs/179-pedal-focus-mount.md).

**실사용 전 확인:** [build151 개발 현황·업데이트 계획·QA 체크리스트](../../docs/178-development-handoff.md). 이 문서는 build151 당시 인수인계이며, 이후 실행 순서는 위 버전 계획을 따른다. 최신 소스와 기존 설치 앱의 버전은 다를 수 있다.
...
**build141은 빠른 모드 전환에서도 최신 포커스 요청을 유지한다.** 자동 attach와 mode 진입을 통합하고, 저장 view는 실제 창 활성화 뒤 포커스를 받도록 개선했다. 빠른 전환의 점 추가 방지와 키보드·입력 보호·재시작, 관련 39개 검사와 Release 48.10초를 확인했다. 모든 timing 조합·물리 청취 완료는 아니다. [검증 기록](../../docs/163-editor-focus-intent.md).

**build140은 MIDI·오디오 편집 진입 후 키보드 포커스를 개선한다.** 실제 방향키·Tab·Enter와 입력 보호를 확인했고 Release 46.68초·관련 테스트 39개를 통과했다. 당시 빠른 모드 전환에 남았던 자동 포커스 경쟁은 위 build141에서 후속 수정·검증했다. [검증과 알려진 한계](../../docs/162-editor-navigation-focus.md).

**build139는 서클마다 편집 화면을 기억한다.** 첫 방문과 재방문을 구분하고 명시적 스텝 요청을 우선하며, 잘못된 수치·이름 입력 중 탐색을 보호한다. 최종 Release 47.96초·관련 검사와 실제 왕복/재시작을 확인했다. source 변경·길이 변화의 모든 조합과 물리 청취는 별도 검증 범위다. [검증 기록](../../docs/161-circle-editor-memory.md).
