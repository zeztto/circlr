...
- 앱 소유 AI socket에서 느린 같은 UID 연결이 다른 요청을 지연시키는 경로를 비차단 수신으로 바꾸고, 완료 요청까지 32개·집계 32MiB·응답 8MiB·기한을 적용한다. 실제 flood 회귀와 독립 보안 재검토는 [build235 QA](qa/0.80-build235.md)에 기록한다. 공개 사용자 Codex 계정 대화는 여전히 G0 NO-GO다.
- 시스템 기본 오디오 입력의 장치 호출을 서명된 앱 소유 자식 프로세스로 격리한다. 자식 PID 소유·시간 제한 정리, 잘못된 응답 거절, CAF 검증 후 원자적 게시와 취소 경계를 구현한다. 장치 UID 직접 선택은 실기기 검증 전까지 부모 HAL 조회 없이 미지원으로 실패한다. 실제 마이크 PCM·macOS 권한 귀속은 아직 확인되지 않았다. 구형 Scarlett 정상 작동을 합격 전제로 삼지 않는다.
- 최종 Swift 회귀 **1,273개·15 skip·실패 0**, MCP Python **47+39개**, 0.80.0/build235 arm64 앱·**7개 helper**의 strict deep 서명과 동봉 데모·Codex kit manifest/source byte 일치를 확인했다. 격리 실제 앱의 700/720/900/1440pt 조작·AX, 중간 섹션 앞/뒤 이동·Undo, 앱 전용 내장 출력의 Play/Stop 시계와 한영 README 가로·세로 전체 창 화면을 확인했다. 게시용 private CAF 복사 중 앱 crash가 나면 일부 파일이 남을 수 있어 정리는 후속 작업으로 남긴다. [build235 QA](qa/0.80-build235.md). 실제 입력 PCM·TCC·물리 청취와 0.80 출고 gate가 남아 정식 tag/Release는 만들지 않는다.
