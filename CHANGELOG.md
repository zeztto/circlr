...
- 궤도 외부로 펼치는 서클 배치와 가로·세로 화면에 대응하는 상단 조작 UI를 개발 중이다. 출고 상태는 [0.50 계획](docs/releases/0.50.0.md)에서 확인한다.
- 한·영 README와 macOS 설치 안내에 현재 ad-hoc 서명·미공증 상태, 다운로드 체크섬 확인 및 Gatekeeper의 ‘그래도 열기’ 절차를 추가했다.
