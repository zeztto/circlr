...
- 가시성을 우선해 궤도·연결선·텍스트·IN/OUT 배지·작업 상태 표시를 개선하고 팔로우 대상/구도와 녹화 background 처리의 첫 구현을 진행했다.
- 텍스트 없는 감상 모드·팔로우 MCP 설정·연속 루프 PCM·반복 오디오 영상 출력·새 곡/템플릿 시작 화면·클래식 MIDI 데모 3곡을 구현 중이다. 루프 전환과 사본 lifecycle, 통합 출고 검증이 남아 있다.
- native QA와 실제 한글 입력은 사용자 승인으로 유예하며 코드·자동 검사 범위를 [실행 계획](docs/releases/0.40.0.md)에 기록한다. 아직 0.40 릴리스가 아니다.
