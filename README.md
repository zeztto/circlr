...
**개발 운영:** [버전별 계획·완료 절차](docs/releases/README.md) · [0.30.0 상세 계획](docs/releases/0.30.0.md) · [이후 로드맵](docs/releases/roadmap.md). 현재 코드는 0.20.0 build152이며, 다음 목표 0.30.0은 아직 계획 단계다. 기능 묶음별 구현·QA·독립 검토·문서·commit/tag/push를 마친 뒤 릴리스한다.

macOS용 앨범·송폼 궤도 편집기. **앨범 → 곡·악장 → 섹션 → MIDI·오디오·악기·이펙터**를 하나의 자유로운 캔버스에서 다룬다.

**최신 소스 build152:** 페달 그래프 준비 중 포커스 요청을 유지하도록 수정했다. 별도 QA 앱에서 모드 전환·재열기 후 클릭 없는 Home/End 선택과 데이터 보존을 확인했다. 전환 직후 입력 타이밍은 남은 확인 항목이며 설치 앱은 교체하지 않았다. [변경·확인 절차](docs/179-pedal-focus-mount.md).
