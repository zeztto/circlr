...
현재 로컬 앱은 **0.17.0**이다. [써클러 앱](dist/써클러.app)을 열어 사용한다. 기존 실행 중인 앱은 저장하고 **⌘Q로 종료한 뒤 다시 열어야** 새 버전이 실행된다. 이전 앱은 `dist/archive/`에 보관한다. [0.17 검증·제한](qa/0.17-review.md) · [향후 상세 개발 계획](docs/25-development-roadmap.md).
