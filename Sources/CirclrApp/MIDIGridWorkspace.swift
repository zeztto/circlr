...
struct MIDIGridWorkspace:View {
    ...
    var body:some View {
        ...
        VStack(alignment:.leading,spacing:10) {
            if store.midiStepMode {
                StepEditor(store:store,topPitch:$topPitch,state:$steps,focusTarget:focusTarget,scroll:$stepScroll,inspectorGap:compact ? 12:20) {
                    noteInspector(compact:compact)
                }
            } else {
                MIDIWorkspaceToolbar(store:store,focusTarget:focusTarget) {
                    Button("선택 보기"){revealSelectedNotes()}.disabled(selected==nil).help("선택한 MIDI 노트로 이동 · F")
                    Spacer(minLength:0)
                    Button{topPitch=max(27,topPitch-12);focusTarget.focus()}label:{Image(systemName:"minus")}.accessibilityLabel("표시 음역 한 옥타브 아래")
    ...
    private func noteInspector(compact:Bool)->some View {
