...
struct InspectorView:View {
    ...
    @ViewBuilder func transition(_ e:FlowEdge)->some View {
        ...
        EffectControls(effect:Binding(get:{e.transition.effect},set:{v in store.updateEdge{$0.transition.effect=v}}))
...
struct EffectControls:View {@Binding var effect:Effect;var allowsAU=false
    var body:some View{VStack(alignment:.leading,spacing:12){StudioChoice("처리",selection:$effect.kind,options:EffectKind.allCases.filter{allowsAU || $0 != .audioUnit}.map{($0,AppStore.effectName($0))});CompactNumber(effect.kind == .gain ? "배율":effect.kind == .pan ? "좌우 · −1–1":"Amount · 0–1",value:$effect.amount);if [.delay,.reverb,.compressor].contains(effect.kind){CompactNumber("Mix / Ratio · 0–0.95",value:$effect.secondary)}}}
}
