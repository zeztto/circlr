...
struct MIDIImportExtent {
    ...
    static func resolve(_ parts:[MIDIImportPart],useID:ID,extendSection:Bool,atBeat:Double,in project:Project)throws->Self {
        ...
        guard parts.reduce(0,{$0+($1.pitchBend?.events.count ?? 0)+($1.sustain?.events.count ?? 0)})<=1_000_000 else{throw CirclrError("가져올 MIDI 표현 이벤트 합계가 1,000,000개를 넘습니다")}
        ...
        for part in parts {
            ...
            try MIDISustainStorage.validate(sustain:part.sustain,pitchBend:part.pitchBend)
            if let expression=part.sustain {
                guard !part.drums else{throw CirclrError("드럼 트랙의 서스테인 가져오기는 아직 지원하지 않습니다. 표현 제외를 선택하세요")}
                let end=atBeat+(expression.events.last?.beat ?? 0)
                guard end.isFinite,end<=131072 else{throw CirclrError("이동한 서스테인 위치가 131072박 한도를 넘습니다")}
        ...
        if !contains(clock) {
            guard extendSection else{throw CirclrError("MIDI 연주 또는 마지막 MIDI 표현 이벤트가 섹션 끝에 닿거나 넘습니다. 섹션 길이 늘리기를 선택하세요")}
