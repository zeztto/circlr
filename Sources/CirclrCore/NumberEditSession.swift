...
public struct NumberEditSession<Context: Equatable> {
    ...
    public func resolve(value: Double, context: Context, range: ClosedRange<Double>, integerOnly: Bool = false) throws -> Double? {
        ...
        guard range.contains(number) else {
            ...
            let unit:String
            switch presentation {case .gainDecibels:unit="dB";case .panPercent,.resonancePercent:unit="%";case .sourceSeconds:unit="초";case .sourceMilliseconds:unit="ms";case .effectValue(let value):unit=value;case .beatPosition:unit="박";case .number:unit=""}
