...
public enum AgentPortEditing {
    public static func snapshot(at node: CircleAddress, in project: Project) throws -> AgentPortSnapshot {
        ...
        let connections = try CirclePortCatalog.connections(in: project).filter { $0.from.node == node || $0.to.node == node }
    ...
    public static func apply(_ request: AgentRequest, to input: Project) throws -> AgentPortEditResult {
        ...
        return .init(project: project, connectionID: id)
    }

    private static func supported(_ address: CircleAddress) throws {
        switch address {
        case .signal, .music, .section, .composition: break
        default: throw CirclrError("실제 음악·섹션·곡·사운드 노드 주소를 사용하세요. 그룹 binding은 아직 제공하지 않습니다")
        }
    }
}
