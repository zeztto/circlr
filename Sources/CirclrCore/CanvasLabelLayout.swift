...
public enum CanvasLabelLayout {
    public static func place(_ requests:[CanvasLabelRequest],within viewport:CGRect,avoiding obstacles:[CGRect]=[])->[CanvasLabelPlacement] {
        ...
        for (_,request) in ordered {
            ...
            guard w>0,h>0,r.isFinite,p.x.isFinite,p.y.isFinite else{continue}
            var origins:[CGPoint]=[]
            ...
            if r<45 {for offset in [h+5,-h-5,2*h+10,-2*h-10] {origins.append(CGPoint(x:p.x+r+9,y:p.y-h/2+offset));origins.append(CGPoint(x:p.x-r-w-9,y:p.y-h/2+offset))}}
            if let rect=origins.map({CGRect(origin:$0,size:request.size)}).first(where:{candidate in viewport.contains(candidate) && !occupied.contains(where:{$0.intersects(candidate)})}) {
