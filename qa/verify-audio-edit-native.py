#!/usr/bin/env python3
"""Capture or exercise audio editing on the exact dedicated 0.18 QA project."""
import argparse
import hashlib
import json
...
OUT=ROOT/'qa/generated/0.18'
EXPECTED=OUT/'audio-QA.circlr'
...
def state():
    s=call('snapshot')
    assert s['runtime']['bundleID']=='com.circlr.hierarchyqa' and s['runtime']['version']=='0.18.0' and s['path']==str(EXPECTED),'Expected dedicated 0.18 QA app and exact copy'
    return s
...
def main():
    ...
    if a.commands:
        ...
        assert next(n for n in before['graph']['nodes'] if n['id']==address['nodeID'])['content'].get('audio')
        op=dict(kind='edit_audio',**address,edit='split',sourceOffset=.5)
        write('apply',{'operations':[op]});after=inspect(state())
        assert len(after['graph']['nodes'])==len(before['graph']['nodes'])+1
        s=state();edit=dict(kind='edit_audio',**address,edit='fade',fadeIn=.1,fadeOut=.1);failed=raw('apply',{'projectID':s['projectID'],'expectedRevision':s['revision'],'operations':[edit,dict(edit,nodeID='missing')]})
        ...
        result['commands']={'splitSourcePreserved':True,'invalidBatchAtomic':True,'undoRestored':True}
    if a.bounce_audio:
        import shutil
        assert not a.export
