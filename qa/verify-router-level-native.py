...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-router-level-native.py before --baseline
  python3 qa/verify-router-level-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/router-level'
...
def capture(name, candidate='final'):
    ...
    expected_build = '91' if candidate == 'baseline' else '92'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build91 package')
