#!/usr/bin/env python3
"""Save/snapshot the isolated arrangement-continuation QA app; never play or record.

  python3 qa/verify-arrangement-continuation-native.py before --candidate final
  python3 qa/verify-arrangement-continuation-native.py reopened --candidate final
...
OUT = q.ROOT / 'qa/generated/arrangement-continuation'
...
def capture(name, candidate='final'):
    ...
    assert injection_path.exists(), 'UI-only continuation QA requires deny-output package'
    ...
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/arrangement-continuation')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/arrangement-continuation.circlr'
