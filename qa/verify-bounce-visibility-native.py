...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-bounce-visibility-native.py before --baseline
  python3 qa/verify-bounce-visibility-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/bounce-visibility'
...
def capture(name, candidate='final'):
    ...
    expected_build = '84' if candidate == 'baseline' else '85'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build84 package')
