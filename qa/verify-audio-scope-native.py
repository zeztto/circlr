...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-audio-scope-native.py before --baseline
  python3 qa/verify-audio-scope-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/audio-scope'
...
def capture(name, candidate='final'):
    ...
    expected_build = '85' if candidate == 'baseline' else '86'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build85 package')
