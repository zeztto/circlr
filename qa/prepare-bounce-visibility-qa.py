#!/usr/bin/env python3
"""Create build84 baseline once; package build85 candidates without resetting music.

  python3 qa/prepare-bounce-visibility-qa.py --baseline
  python3 qa/prepare-bounce-visibility-qa.py --candidate final

The common packager copies authored assets and creates its usual shared second use.
Disconnected audio cases are prepared separately through the app, never here.
"""
import argparse
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build84 release')
    mode.add_argument('--candidate', help='New build85 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build84 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '84' if args.baseline else '85'
