#!/usr/bin/env python3
"""Snapshot/save only the exact isolated synth QA app. Never play, audition or record."""
import argparse
...
p = module('synth_qa_package', 'prepare-synth-workspace-qa.py')
q = module('synth_qa_native', 'verify-integration-native.py')
...
def capture(name, candidate, save=False):
    ...
    assert package['build'] in ['118', '119']
