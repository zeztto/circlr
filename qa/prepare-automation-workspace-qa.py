...
FIXTURE = SOURCE.with_name('automation-workspace.circlr')
...
def main():
    ...
    for worker in ['circlr-output-worker', 'circlr-au-effect-worker', 'circlr-au-instrument-worker', 'circlr-output-device-catalog']:
    ...
    shutil.copy2(ROOT / '.build/integration-release/release/circlr-output-device-catalog', APP / 'Contents/MacOS/circlr-output-device-catalog')
    ...
    subprocess.run(['codesign', '--force', '--sign', '-', str(APP / 'Contents/MacOS/circlr-output-device-catalog')], check=True)
    ...
    subprocess.run(['codesign', '--verify', '--strict', str(APP / 'Contents/MacOS/circlr-output-device-catalog')], check=True)
