#!/usr/bin/env python3
"""Create an exclusive authored step target fixture or package prebuilt UI-only QA binaries.

Seed: python3 qa/prepare-step-target-qa.py
Package: python3 qa/prepare-step-target-qa.py --candidate baseline121 --binary-dir PATH --info-plist PATH
...
OUT = ROOT / 'qa/generated/step-target'
SOURCE = Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/rhythm-scope.circlr'
FIXTURE = SOURCE.with_name('step-target.circlr')
PROJECT_ID = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/step-target')).upper()
NODE = 'rhythm-midi:7E0D9016-E91D-4325-B807-033F233E32EA'
...
_spec = importlib.util.spec_from_file_location('step_shared_helpers', Path(__file__).with_name('prepare-synth-workspace-qa.py'))
...
def seed():
    ...
    track_id = node['content']['rhythmMIDI']['trackID']
    pattern = next(x for x in project['patterns'] if x['id'] == project['global']['rhythm']['patternID'])
    assert pattern['trackID'] == track_id and len(pattern['notes']) > 0
    lane = next(x for x in use['addedLanes'] if x['trackID'] == track_id and x['notes'])
    midi_node = next(x for x in use['graphEdits']['addedNodes'] if x['content'].get('midi', {}).get('laneID') == lane['id'])
    section = next(x for x in project['sections'] if x['id'] == use['sectionID'])
    assert section['settings']['rhythm']['source'] == use['settings']['rhythm']['source'] == node['settings']['rhythm']['source'] == 'inherit'
    assert any(t['id'] == track_id for t in project['tracks'])
    project.update(id=PROJECT_ID, name='스텝 편집 대상 유지 검증')
    ...
    exclusive(baseline / 'scenario.json', dict(projectID=PROJECT_ID, fixture=str(FIXTURE), source=str(SOURCE),
        ...
        arrangementID=arrangement['id'], useID=use['id'], nodeID=NODE, trackID=track_id, patternID=pattern['id'], midiNodeID=midi_node['id'], laneID=lane['id'],
        assets=project['assets'], firstTrack=original_tracks[0], physicalAudioAttempts=0,
        scope='Authored rhythm-scope copy; all musical content preserved; inherited shared rhythm MIDI selected with ordinary MIDI on the same track'))
...
def package(candidate, binary_dir, info_path):
    ...
    assert info['CFBundleVersion'] in ['121', '122'] and info['CFBundleShortVersionString'] == '0.20.0'
