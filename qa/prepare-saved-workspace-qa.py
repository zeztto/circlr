#!/usr/bin/env python3
"""Package build 67 with authored section branches and a separate owner."""
import copy
import hashlib
...
packager.NAME='saved-workspace';packager.BUILD='67'
packager.OUT=packager.ROOT/'qa/generated/saved-workspace';packager.APP=packager.OUT/'써클러 통합 검증.app'
packager.FIXTURE=packager.SOURCE.with_name('saved-workspace.circlr')


def identifier(name):return str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/saved-workspace/'+name)).upper()
...
if __name__=='__main__':
    ...
    sources=['Resources/Info.plist','Sources/CirclrCore/ConnectionWorkspaceState.swift','Sources/CirclrCore/StudioWorkspace.swift',
             'Sources/CirclrCore/HierarchyScene.swift','Sources/CirclrCore/CircleHistory.swift','Sources/CirclrApp/AppStore.swift',
             'Sources/CirclrApp/AlbumCanvas.swift','Sources/CirclrApp/SavedWorkspace.swift','Sources/CirclrApp/PortConnectionsEditor.swift','Sources/CirclrApp/InlineCircleEditor.swift']
