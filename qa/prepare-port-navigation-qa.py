#!/usr/bin/env python3
"""Create build89 baseline once; package build90 candidates without resetting music.

  python3 qa/prepare-port-navigation-qa.py --baseline
  python3 qa/prepare-port-navigation-qa.py --candidate final

The common packager copies the two authored assets, then the baseline imports the
preserved track-shortcut prepared document under a new project identity.
"""
import argparse
import copy
import hashlib
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build89 release')
    mode.add_argument('--candidate', help='New build90 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build89 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '89' if args.baseline else '90'
    ...
    if args.baseline:
        ...
        evidence_path = p.ROOT / 'qa/generated/track-shortcut/baseline/prepared.json'
        evidence = json.loads(evidence_path.read_text())
        assert evidence['state']['revision'] == evidence['manifest']['musicRevision'] == 16
        assert evidence['state']['projectID'] == '9BCB7FE4-589A-53A2-9339-CC91A55E776C'
        ...
        with (p.OUT / 'fixture-origin.json').open('x') as file:
            json.dump({'sourceEvidence': str(evidence_path),
                       ...
                       'newProjectID': project['id'], 'sourceRevision': 16}, file, indent=2)
