#!/usr/bin/env python3
"""Create build88 baseline once; package build89 candidates without resetting music.

  python3 qa/prepare-track-shortcut-qa.py --baseline
  python3 qa/prepare-track-shortcut-qa.py --candidate final

The common packager copies authored assets and creates its usual shared second use.
Track shortcut ambiguity cases are prepared separately through the app, never here.
"""
import argparse
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build88 release')
    mode.add_argument('--candidate', help='New build89 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build88 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '88' if args.baseline else '89'
