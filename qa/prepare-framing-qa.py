...
OUT = ROOT / 'qa/generated/framing'
APP = OUT / '써클러 통합 검증.app'
SOURCE = Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/studio.circlr'
FIXTURE = SOURCE.with_name('framing.circlr')


def main():
    assert not APP.exists() and not FIXTURE.exists(), 'Preserve previous QA artifacts'
    ...
    assert info['CFBundleShortVersionString'] == '0.20.0' and info['CFBundleVersion'] == '24'
    ...
    project['id'] = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/framing')).upper()
    ...
    result = dict(app=str(APP), fixture=str(FIXTURE), projectID=project['id'], build='24',
