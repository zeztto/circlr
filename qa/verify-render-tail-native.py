...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-render-tail-native.py before --baseline
  python3 qa/verify-render-tail-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/render-tail'
...
def capture(name, candidate='final'):
    ...
    expected_build = '86' if candidate == 'baseline' else '87'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build86 package')
