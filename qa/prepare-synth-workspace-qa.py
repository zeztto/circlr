#!/usr/bin/env python3
"""Create an exclusive authored synth fixture or package prebuilt UI-only QA binaries.

Seed: python3 qa/prepare-synth-workspace-qa.py
Package: python3 qa/prepare-synth-workspace-qa.py --candidate baseline118 --binary-dir PATH --info-plist PATH
...
OUT = ROOT / 'qa/generated/synth-workspace'
SOURCE = Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/studio.circlr'
FIXTURE = SOURCE.with_name('synth-workspace.circlr')
PROJECT_ID = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/synth-workspace')).upper()
...
def seed():
    ...
    project.update(id=PROJECT_ID, name='신스 작업 공간 검증')
...
def package(candidate, binary_dir, info_path):
    ...
    assert info['CFBundleVersion'] in ['118', '119'] and info['CFBundleShortVersionString'] == '0.20.0'
