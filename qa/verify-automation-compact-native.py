#!/usr/bin/env python3
"""Save/snapshot the isolated automation-compact QA app; never play or record.

  python3 qa/verify-automation-compact-native.py compact --candidate final2
  python3 qa/verify-automation-compact-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/automation-compact'
...
def capture(name, candidate='final'):
    ...
    expected_build = '104' if candidate == 'baseline' else '105'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/automation-compact')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/automation-compact.circlr'
