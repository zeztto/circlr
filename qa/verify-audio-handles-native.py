#!/usr/bin/env python3
"""Save/snapshot the isolated audio-handles QA app; never play or record.

  python3 qa/verify-audio-handles-native.py before --candidate final
  python3 qa/verify-audio-handles-native.py reopened --candidate final
...
OUT = q.ROOT / 'qa/generated/audio-handles'
...
def capture(name, candidate='final', allow_render_asset=False):
    ...
    assert injection_path.exists(), 'UI-only audio-handles QA requires deny-output package'
    ...
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/audio-handles')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/audio-handles.circlr'
