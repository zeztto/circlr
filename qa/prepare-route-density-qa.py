#!/usr/bin/env python3
"""Create build94 baseline once; package build95 candidates without resetting music.

  python3 qa/prepare-route-density-qa.py --baseline
  python3 qa/prepare-route-density-qa.py --candidate final

The common packager copies the two authored assets, then the baseline imports the
preserved console-height reopened document under a new project identity.
"""
import argparse
import copy
import hashlib
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build94 release')
    mode.add_argument('--candidate', help='New build95 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build94 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '94' if args.baseline else '95'
    ...
    if args.baseline:
        ...
        evidence_path = p.ROOT / 'qa/generated/console-height/final-shortcuts/reopened.json'
        evidence = json.loads(evidence_path.read_text())
        assert evidence['state']['revision'] == evidence['manifest']['musicRevision'] == 62
        assert evidence['state']['projectID'] == 'F0E5C644-2902-5659-81B9-74F2AE5A0AC6'
        ...
        with (p.OUT / 'fixture-origin.json').open('x') as file:
            json.dump({'sourceEvidence': str(evidence_path),
                       ...
                       'newProjectID': project['id'], 'sourceRevision': 62}, file, indent=2)
