...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-route-density-native.py before --baseline
  python3 qa/verify-route-density-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/route-density'
...
def capture(name, candidate='final'):
    ...
    expected_build = '94' if candidate == 'baseline' else '95'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build94 package')
