...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-midi-compact-native.py before --baseline
  python3 qa/verify-midi-compact-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/midi-compact'
...
def capture(name, candidate='final'):
    ...
    expected_build = '99' if candidate == 'baseline' else '100'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/midi-compact')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/midi-compact.circlr'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build99 package')
