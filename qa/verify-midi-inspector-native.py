#!/usr/bin/env python3
"""Capture only the owned build 69 project; never starts playback or recording."""
...
qa.OUT=qa.ROOT/'qa/generated/midi-inspector'
qa.FIXTURE=qa.FIXTURE.with_name('midi-inspector.circlr')
qa.PROJECT_ID=str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/midi-inspector')).upper()

if __name__=='__main__':
    assert len(sys.argv) in (2,3) and re.fullmatch('[a-z0-9-]{1,64}',sys.argv[1])
    assert len(sys.argv)==2 or sys.argv[2]=='--save'
    before=qa.state();assert before['runtime']['build']=='69'
