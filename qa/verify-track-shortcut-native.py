...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-track-shortcut-native.py before --baseline
  python3 qa/verify-track-shortcut-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/track-shortcut'
...
def capture(name, candidate='final'):
    ...
    expected_build = '88' if candidate == 'baseline' else '89'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build88 package')
