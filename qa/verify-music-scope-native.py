...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-music-scope-native.py before --baseline
  python3 qa/verify-music-scope-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/music-scope'
...
def capture(name, candidate='final'):
    ...
    expected_build = '92' if candidate == 'baseline' else '93'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build92 package')
