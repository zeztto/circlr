#!/usr/bin/env python3
"""Save/snapshot the isolated arrangement-input QA app; never play or record.

  python3 qa/verify-arrangement-input-native.py before --candidate final
  python3 qa/verify-arrangement-input-native.py reopened --candidate final
...
OUT = q.ROOT / 'qa/generated/arrangement-input'
...
def capture(name, candidate='final'):
    ...
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/arrangement-input')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/arrangement-input.circlr'
