#!/usr/bin/env python3
"""Save exclusive build109 QA snapshots; never play, record, or change preferences.

python3 qa/verify-output-preferences-native.py before --candidate success
"""
import argparse
import uuid
import hashlib
...
OUT = q.ROOT / 'qa/generated/output-preferences'


def capture(name, candidate='success'):
    assert re.fullmatch('[a-z0-9-]{1,64}', name)
    assert re.fullmatch('[a-z0-9-]{1,32}', candidate)
    directory = OUT / candidate
    package = json.loads((directory / 'package.json').read_text())
    expected_build = '109'
    assert re.fullmatch(r'(success|delay|failure|missing)[0-9]*', candidate)
    injection = json.loads((directory / 'qa-injection.json').read_text())
    assert injection['qaOnlyMock'] is True and injection['candidate'] == candidate
    assert injection.get('scenario', candidate) in ['success','delay','failure','missing']
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/output-preferences')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/output-preferences.circlr'
