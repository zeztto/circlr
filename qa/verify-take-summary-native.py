#!/usr/bin/env python3
"""Save/snapshot the isolated take-summary QA app; never play or record.

  python3 qa/verify-take-summary-native.py compact --candidate final2
  python3 qa/verify-take-summary-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/take-summary'
...
def capture(name, candidate='final'):
    ...
    expected_build = '106' if candidate == 'baseline' else '107'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/take-summary')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/take-summary.circlr'
