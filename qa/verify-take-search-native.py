#!/usr/bin/env python3
"""Save/snapshot the isolated take-search QA app; never play or record.

  python3 qa/verify-take-search-native.py compact --candidate final2
  python3 qa/verify-take-search-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/take-search'
...
def capture(name, candidate='final'):
    ...
    expected_build = '107' if candidate == 'baseline' else '108'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/take-search')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/take-search.circlr'
