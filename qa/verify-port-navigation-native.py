...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-port-navigation-native.py before --baseline
  python3 qa/verify-port-navigation-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/port-navigation'
...
def capture(name, candidate='final'):
    ...
    expected_build = '89' if candidate == 'baseline' else '90'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build89 package')
