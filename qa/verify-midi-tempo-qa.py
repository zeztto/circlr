#!/usr/bin/env python3
"""Snapshot/save exact build131 MIDI tempo capture; musical changes require independent comparison."""
import argparse
import copy
import hashlib
...
OUT=ROOT/'qa/generated/midi-tempo'
FIXTURE=Path.home()/'Library/Application Support/circlr-integration-qa/fixtures/midi-tempo.circlr'
PROJECT_ID='28D12204-EBBD-5035-BAA5-9B9C5BBADA3E'
...
spec=importlib.util.spec_from_file_location('midi_tempo_transport',Path(__file__).with_name('verify-integration-native.py'))
...
def state(expected_revision):
    s=q.state();assert s['runtime']['build']=='131' and s['revision']==expected_revision
...
def capture(name,candidate,expected_revision):
    ...
    assert package['app']==str(app) and str(package['build'])=='131'
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('name');parser.add_argument('--candidate',default='final131')
    parser.add_argument('--expected-revision',type=int,required=True)
    parser.add_argument('--unsupported',action='store_true',help='Explicitly capture the dedicated tempo-follow rejection fixture')
    args=parser.parse_args()
    if args.unsupported:
        FIXTURE=FIXTURE.with_name('midi-tempo-unsupported.circlr')
        PROJECT_ID='E31B5DA3-67C4-544D-A83E-F6C2FB7359BB'
