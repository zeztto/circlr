#!/usr/bin/env python3
"""Package build 66 with authored section branches and a separate owner."""
import copy
import hashlib
...
packager.NAME='workspace-return';packager.BUILD='66'
packager.OUT=packager.ROOT/'qa/generated/workspace-return';packager.APP=packager.OUT/'써클러 통합 검증.app'
packager.FIXTURE=packager.SOURCE.with_name('workspace-return.circlr')


def identifier(name):return str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/workspace-return/'+name)).upper()
...
if __name__=='__main__':
    ...
    sources=['Resources/Info.plist','Sources/CirclrCore/ConnectionWorkspaceState.swift','Sources/CirclrApp/PortConnectionsEditor.swift',
             'Sources/CirclrApp/AppStore.swift','Sources/CirclrApp/AlbumWorkspace.swift','Sources/CirclrApp/InlineEditorHeader.swift',
             'Sources/CirclrApp/MusicContextEditor.swift','Sources/CirclrApp/StudioModeButton.swift','Sources/CirclrApp/InlineCircleEditor.swift']
