#!/usr/bin/env python3
"""Snapshot/save only the exact isolated console preferences QA app. Never play, audition or record."""
import argparse
...
p = module('console_qa_package', 'prepare-console-preferences-qa.py')
q = module('console_qa_native', 'verify-integration-native.py')
...
def capture(name, candidate, save=False):
    ...
    def state():
        ...
        assert value['revision'] == seed['musicRevision'], 'Console actions must not modify music'
    ...
    if save:
        ...
        assert {k:v for k,v in manifest.items() if k not in ignored} == {k:v for k,v in seed.items() if k not in ignored}, 'Console QA must preserve musical content'
