#!/usr/bin/env python3
"""Create an exclusive authored effect access fixture or package prebuilt UI-only QA binaries.

Seed: python3 qa/prepare-effect-access-qa.py
Package: python3 qa/prepare-effect-access-qa.py --candidate baseline120 --binary-dir PATH --info-plist PATH
...
OUT = ROOT / 'qa/generated/effect-access'
SOURCE = Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/effect-editing.circlr'
FIXTURE = SOURCE.with_name('effect-access.circlr')
PROJECT_ID = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/effect-access')).upper()
NODE = '0D6672CC-2AD9-4855-82E8-6937D940961E'
...
_spec = importlib.util.spec_from_file_location('effect_shared_helpers', Path(__file__).with_name('prepare-synth-workspace-qa.py'))
...
def seed():
    ...
    assert arrangement['id'] == '95B906AF-5A06-43F0-BF79-8EDB26A8973F'
    assert use['id'] == 'C958E497-716C-5DA2-B597-B55375AE2A90'
    original_effect = dict(node['content']['effect']['_0'])
    assert original_effect == dict(kind='lowpass', amount=0.5, secondary=0.25)
    node['content']['effect']['_0'] = dict(kind='compressor', amount=0.5, secondary=0.25)
    project.update(id=PROJECT_ID, name='이펙트 접근성 검증')
    ...
    exclusive(baseline / 'scenario.json', dict(projectID=PROJECT_ID, fixture=str(FIXTURE), source=str(SOURCE),
        ...
        arrangementID=arrangement['id'], useID=use['id'], nodeID=NODE, sourceEffect=original_effect, seedEffect=node['content']['effect']['_0'],
        assets=project['assets'], firstTrack=original_tracks[0], physicalAudioAttempts=0,
        scope='Authored effect-editing copy; only target lowpass replaced with compressor amount0.5 secondary0.25; tracks/assets unchanged; seed musicRevision inherited'))
...
def package(candidate, binary_dir, info_path):
    ...
    assert info['CFBundleVersion'] in ['120', '121'] and info['CFBundleShortVersionString'] == '0.20.0'
