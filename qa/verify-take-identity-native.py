#!/usr/bin/env python3
"""Save/snapshot the isolated take-identity QA app; never play or record.

  python3 qa/verify-take-identity-native.py compact --candidate final2
  python3 qa/verify-take-identity-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/take-identity'
...
def capture(name, candidate='final'):
    ...
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/take-identity')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/take-identity.circlr'
