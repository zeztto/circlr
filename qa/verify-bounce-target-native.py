#!/usr/bin/env python3
"""Owned native bounce target QA helpers; never starts an audio device."""
...
qa.OUT=qa.ROOT/'qa/generated/bounce-target'
p=json.loads((qa.OUT/'package.json').read_text());qa.PROJECT_ID=p['projectID'];qa.FIXTURE=Path(p['fixture'])
def capture(name):
 s=qa.state();assert s['runtime']['build']=='80' and s['output']['attempts']==s['audition']['attempts']==0
