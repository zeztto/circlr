...
"""Save/snapshot the exact baseline or candidate QA app; never play or record.
  ...
  python3 qa/verify-console-height-native.py before --baseline
  python3 qa/verify-console-height-native.py disconnected --candidate final
...
OUT = q.ROOT / 'qa/generated/console-height'
...
def capture(name, candidate='final'):
    ...
    expected_build = '93' if candidate == 'baseline' else '94'
...
if __name__ == '__main__':
    ...
    if args.candidate == 'baseline':
        parser.error('Use --baseline for the build93 package')
