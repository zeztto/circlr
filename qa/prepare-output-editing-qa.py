...
def main():
    ...
    if args.candidate:
        ...
        assert current['id'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/output-editing')).upper()
    ...
    assert info['CFBundleShortVersionString'] == '0.20.0' and info['CFBundleVersion'] == '33'
    ...
    if not args.candidate:
        ...
        project['id'] = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/output-editing')).upper()
        second=copy.deepcopy(project['arrangements'][0]['uses'][0])
        second['id']=str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/output-editing/second-use')).upper()
    ...
    project['id'] = str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/output-editing')).upper()
    result = dict(app=str(APP), fixture=str(FIXTURE), projectID=project['id'], build='33',
