#!/usr/bin/env python3
"""Capture only the owned build 63 project; never starts playback or recording."""
...
qa.OUT=qa.ROOT/'qa/generated/arrangement-search'
qa.FIXTURE=qa.FIXTURE.with_name('arrangement-search.circlr')
qa.PROJECT_ID=str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/arrangement-search')).upper()

if __name__=='__main__':
    assert len(sys.argv) in (2,3) and re.fullmatch('[a-z0-9-]{1,64}',sys.argv[1])
    assert len(sys.argv)==2 or sys.argv[2]=='--save'
    before=qa.state();assert before['runtime']['build']=='63'
