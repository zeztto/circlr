#!/usr/bin/env python3
"""Save/snapshot the isolated audio-compact QA app; never play or record.

  python3 qa/verify-audio-compact-native.py compact --candidate final2
  python3 qa/verify-audio-compact-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/audio-compact'
...
def capture(name, candidate='final'):
    ...
    expected_build = '102' if candidate == 'baseline' else '103'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/audio-compact')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/audio-compact.circlr'
