#!/usr/bin/env python3
"""Create build92 baseline once; package build93 candidates without resetting music.

  python3 qa/prepare-music-scope-qa.py --baseline
  python3 qa/prepare-music-scope-qa.py --candidate final

The common packager copies the two authored assets, then the baseline imports the
preserved router-level reopened document under a new project identity.
"""
import argparse
import copy
import hashlib
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build92 release')
    mode.add_argument('--candidate', help='New build93 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build92 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '92' if args.baseline else '93'
    ...
    if args.baseline:
        ...
        evidence_path = p.ROOT / 'qa/generated/router-level/final/reopened.json'
        evidence = json.loads(evidence_path.read_text())
        assert evidence['state']['revision'] == evidence['manifest']['musicRevision'] == 54
        assert evidence['state']['projectID'] == 'BEBD7202-BE4A-5FA1-95A5-880069F80993'
        ...
        with (p.OUT / 'fixture-origin.json').open('x') as file:
            json.dump({'sourceEvidence': str(evidence_path),
                       ...
                       'newProjectID': project['id'], 'sourceRevision': 54}, file, indent=2)
