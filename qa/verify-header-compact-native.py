#!/usr/bin/env python3
"""Save/snapshot the isolated header QA app; never play or record.

  python3 qa/verify-header-compact-native.py compact --candidate final2
  python3 qa/verify-header-compact-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/header-compact'
...
def capture(name, candidate='final'):
    ...
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/header-compact')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/header-compact.circlr'
