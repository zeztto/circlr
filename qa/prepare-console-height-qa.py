#!/usr/bin/env python3
"""Create build93 baseline once; package build94 candidates without resetting music.

  python3 qa/prepare-console-height-qa.py --baseline
  python3 qa/prepare-console-height-qa.py --candidate final

The common packager copies the two authored assets, then the baseline imports the
preserved music-scope reopened document under a new project identity.
"""
import argparse
import copy
import hashlib
...
def main():
    ...
    mode.add_argument('--baseline', action='store_true', help='First creation only, from build93 release')
    mode.add_argument('--candidate', help='New build94 package folder; preserve the existing fixture')
    args = parser.parse_args()
    if args.candidate:
        assert re.fullmatch('[a-z0-9-]{1,32}', args.candidate)
        assert args.candidate != 'baseline', 'baseline is reserved for the original build93 package'
        assert (OUT / 'baseline/package.json').is_file(), 'Create --baseline first'
    p.NAME = NAME
    p.BUILD = '93' if args.baseline else '94'
    ...
    if args.baseline:
        ...
        evidence_path = p.ROOT / 'qa/generated/music-scope/final/reopened.json'
        evidence = json.loads(evidence_path.read_text())
        assert evidence['state']['revision'] == evidence['manifest']['musicRevision'] == 62
        assert evidence['state']['projectID'] == 'FAB1D86A-A05B-5F0E-90D4-61A6AC9111EE'
