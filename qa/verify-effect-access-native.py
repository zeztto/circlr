#!/usr/bin/env python3
"""Snapshot/save only the exact isolated effect access QA app. Never play, audition or record."""
import argparse
...
p = module('effect_qa_package', 'prepare-effect-access-qa.py')
q = module('effect_qa_native', 'verify-integration-native.py')
...
def capture(name, candidate, save=False):
    ...
    assert package['build'] in ['120', '121']
    ...
    if save:
        ...
        # Permit only this effect payload and its node gain to change during native editing.
        def preserved(project):
            import copy
            value = copy.deepcopy(project)
            for key in ['hierarchyView', 'layoutRevision', 'modifiedAt', 'musicRevision']:
                value.pop(key, None)
            arrangement = next(a for a in value['arrangements'] if a['id'] == scenario['arrangementID'])
            use = next(u for u in arrangement['uses'] if u['id'] == scenario['useID'])
            node = next(n for n in use['graphEdits']['addedNodes'] if n['id'] == p.NODE)
            node['content']['effect']['_0'] = None
            node['gain'] = None
            return value
        assert preserved(manifest) == preserved(seed), 'Non-target musical data changed'
