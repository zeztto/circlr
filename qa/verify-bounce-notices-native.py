#!/usr/bin/env python3
"""Save/snapshot the isolated bounce-notices QA app; never play or record.

  python3 qa/verify-bounce-notices-native.py compact --candidate final2
  python3 qa/verify-bounce-notices-native.py reopened --candidate final2
...
OUT = q.ROOT / 'qa/generated/bounce-notices'
...
def capture(name, candidate='final'):
    ...
    expected_build = '105' if candidate == 'baseline' else '106'
    assert package['build'] == expected_build
    assert package['projectID'] == str(uuid.uuid5(uuid.NAMESPACE_URL, 'circlr-integration-qa/bounce-notices')).upper()
    q.FIXTURE = Path(package['fixture'])
    assert q.FIXTURE == Path.home() / 'Library/Application Support/circlr-integration-qa/fixtures/bounce-notices.circlr'
