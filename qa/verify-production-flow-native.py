#!/usr/bin/env python3
"""Capture the exact production-flow QA app; preserve raw manifests for independent semantic audit.
This checks identity, no-I/O and asset integrity, not correctness of musical edits."""
import argparse
...
p = module('production_flow_qa_package', 'prepare-production-flow-qa.py')
q = module('production_flow_qa_native', 'verify-integration-native.py')


def capture(name, candidate, save=False):
    assert re.fullmatch('[a-z0-9-]{1,64}', name)
    assert candidate == 'current122'
    ...
    def state():
        ...
        assert value['output']['phase'] == value['audition']['phase'] == 'idle'
        assert value['audition']['heldNotes'] == 0
    ...
    if save:
        ...
        original = {a['id']: a for a in seed['assets']}
        current = {a['id']: a for a in manifest['assets']}
        assert len(original) == 2 and len(current) == len(manifest['assets'])
        assert all(current.get(key) == asset for key, asset in original.items())
        paths = set()
        for asset in manifest['assets']:
            path = Path(asset['path'])
            assert path.parts and not path.is_absolute() and '..' not in path.parts
            assert path not in paths, 'Asset paths must be distinct'
            paths.add(path)
            actual = p.FIXTURE / path
            assert actual.resolve().is_relative_to(p.FIXTURE.resolve()), 'Asset must stay inside fixture'
            assert p.digest(actual) == asset['checksum']
        result['newAssetIDs'] = sorted(set(current) - set(original))
        result['validationScope'] = 'Identity, no-I/O and asset integrity only; exact musical changes require independent snapshot comparison.'
