#!/usr/bin/env python3
"""Snapshot/save exact build130 automation space capture; musical changes require independent comparison."""
import argparse
import copy
import hashlib
...
OUT=ROOT/'qa/generated/automation-space'
FIXTURE=Path.home()/'Library/Application Support/circlr-integration-qa/fixtures/automation-space.circlr'
PROJECT_ID='A130D349-48CD-5826-B947-0C3B18CAAF03'
...
spec=importlib.util.spec_from_file_location('automation_space_transport',Path(__file__).with_name('verify-integration-native.py'))
...
def state(expected_revision):
    s=q.state();assert s['runtime']['build']=='130' and s['revision']==expected_revision
...
def capture(name,candidate,expected_revision):
    ...
    assert package['app']==str(app) and str(package['build'])=='130'
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('name');parser.add_argument('--candidate',default='final130')
