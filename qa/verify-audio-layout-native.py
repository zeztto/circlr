#!/usr/bin/env python3
"""Read/save the owned audio-layout project; no audio output or recording."""
...
qa.OUT=qa.ROOT/'qa/generated/audio-layout';qa.FIXTURE=qa.FIXTURE.with_name('audio-layout.circlr')
qa.PROJECT_ID=str(uuid.uuid5(uuid.NAMESPACE_URL,'circlr-integration-qa/audio-layout')).upper()
if __name__=='__main__':
    assert len(sys.argv)==2 and re.fullmatch('[a-z0-9-]{1,64}',sys.argv[1])
    assert qa.state()['runtime']['build']=='73'
