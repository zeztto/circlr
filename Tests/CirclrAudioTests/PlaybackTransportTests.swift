...
@MainActor final class PlaybackTransportTests:XCTestCase {
    ...
    func testLoopUsesSingleBackendRunAndMonotonicClock()async throws {
        ...
        let cycle=try XCTUnwrap(playback.loopPCM)
        let visual=playback.visualState(atElapsed:cycle.duration+0.25)
        XCTAssertEqual(visual.seconds,0.25,accuracy:0.000001)
        XCTAssertEqual(visual.prepared?.plan.duration,input.plan.duration)
        XCTAssertEqual(playback.musicalSeconds(atElapsed:cycle.duration+0.25),0.25,accuracy:0.000001)
        XCTAssertEqual(playback.musicalSeconds(atElapsed:cycle.duration*2+0.25),0.25,accuracy:0.000001)
        XCTAssertEqual(playback.musicalSeconds(atElapsed:.nan),0)
        XCTAssertEqual(playback.seconds,0.25,"Looking ahead must not advance the live transport")
