...
final class SoundSelectionTests:XCTestCase {
    ...
    func testUnicodeVendorAndKindSearch() {
        let entries=SoundSelection.instruments([plugin("Gláss Keys")])
        ...
        XCTAssertEqual(SoundSelection.search(entries,query:"sound bank").map(\.id),[.soundBank])
    ...
    func testInstrumentAndEffectCatalogsRejectOtherComponentTypes()throws {
        ...
        let instruments=SoundSelection.instruments(all),effects=SoundSelection.effects(all)
    ...
    func testDuplicateNamesKeepDistinctIDsAndDuplicateComponentsAreRemoved() {
        ...
        let rows=SoundSelection.search(SoundSelection.instruments([a,b,a]),query:"Nordic")
    ...
    func testSameSynthPreservesEveryCustomizedValueAndOldEngine()throws {
        ...
        let result=try SoundSelection.instrument(.synth(.pad),current:current,catalog:SoundSelection.instruments([]))
    ...
    func testDifferentSynthUsesVoiceDefaultsPreservingOtherModes()throws {
        ...
        let before=current
        let result=try SoundSelection.instrument(.synth(.bass),current:current,catalog:SoundSelection.instruments([]))
        ...
        let bank=try SoundSelection.instrument(.soundBank,current:result,catalog:SoundSelection.instruments([]))
    ...
    func testSameAudioUnitKeepsSerializedStateAndDifferentUnitReplacesIt()throws {
        let a=plugin(),b=plugin(subtype:2),catalog=SoundSelection.instruments([a,b])
    ...
    func testReturningToAnInactiveSoundRestoresItsSavedSettings()throws {
        let unit=plugin(),catalog=SoundSelection.instruments([unit])
        ...
        let bank=try SoundSelection.instrument(.soundBank,current:current,catalog:catalog)
